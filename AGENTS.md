...
- Tessera 产品内的 Agent 操作遵循当前执行器的资源授权、路径与版本保护、原子提交和审计要求，具体见 [Skill 权限边界](docs/architecture/skill-system.md#权限边界)。普通工作区 `edit/write` 不逐次审批；旧 Diff/批准保留历史兼容，MCP 的显式信任与逐次人工批准继续保留。
- 上述产品运行时边界不要求维护本仓库的编码助手对用户已请求的常规本地修改另行逐次确认。复用当前任务中仍有效的授权与决定；用户明确要求先审阅的任务和具体操作的明确批准要求继续遵守。
- 第三方依赖必须遵守许可证，不复制许可证不兼容的源码。

## 完成检查

完成检查适用于实际改动；纯审阅不得为检查而执行写入命令。实际改动结束前执行以下五项检查：
...
本次改动引起的失败应修复后复验，其他独立检查继续进行。环境阻塞或既有失败须说明证据、影响与未验证项，不将其报告为全部通过；能够在任务范围内修复时继续修复，不以首次失败作为任务结束条件。检查通过后，无新改动或新证据不重复执行。

改动结束前还要检查 Header、最近的 `.folder.md`、文档链接和能力状态是否同步。Header 字段与豁免以本文件为唯一事实源；契约未变化且 Header 准确时无需为了产生文档改动而重写。
