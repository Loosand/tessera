/**
 * [INPUT]: Electron 窄桥、任务模式、当前文档提示、当前模型选择、含工具/审批的版本化历史消息、联网/思考开关与 AI SDK React 状态机
 * [OUTPUT]: 可独立验证且支持断开重连的 ElectronChatTransport、完整 UIMessage 往返、审批后自动续轮与通过类型化 IPC 消费增量事件的 useChat 封装
...
export function toTaskMessages(
  ...
  return messages.map((message) => {
    ...
    const metadata = message.metadata as TaskMessageMetadata | undefined
