/**
 * [INPUT]: 已解析 TaskRunPolicy、模型上下文上限、AI SDK LanguageModel、当前请求工具集合、领域 instructions、Telemetry、生命周期/ContextManifest 观测回调与工具分组
 * [OUTPUT]: 保留具体 ToolSet 类型、通过 callOptionsSchema/prepareCall 动态收窄推理、工具、安全护栏和 Skill instructions、逐步执行上下文预算检查，并以 AI SDK 原生生命周期产出统一运行指标的 ToolLoopAgent
      ...
      return {
        ...
        prepareStep: ({ messages, stepNumber }) => {
          ...
          const phaseInstruction =
            policy?.mode === "plan"
              ...
              : policy?.mode === "evidence"
                ? "运行时要求：已经尝试深读来源但尚未登记证据。优先用一次 record-research-evidence 批量登记最多 12 条短原文证据；如果当前运行没有该来源正文或正文不足，可以调用 read-web-source 重新深读已发现来源；合理重读后仍无可核查材料时，调用 finalize-research 以 partial 如实保留限制。不要继续盲目搜索或直接输出最终答复。"
                    ...
                    : policy?.mode === "finalize-partial"
                      ...
                      : policy?.mode === "final-answer"
                        ...
                        : policy
                          ...
                          : ""
          const stepInstructions = [instructions, phaseInstruction].filter(Boolean).join("\n\n")
          const stepActiveTools = policy?.activeTools ?? activeTools
          const manifest = createTaskContextManifest({
            activeToolNames: stepActiveTools,
          ...
          return {
            ...
            ...(policy ? { toolChoice: policy.toolChoice } : {}),
