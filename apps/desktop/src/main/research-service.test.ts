 ...
 * [OUTPUT]: SSRF 地址拒绝、正文提取、读取失败、证据逐字约束、完整交叉核验与预算部分完成的回归验证
...
describe("研究领域完成门槛", () => {
  it("要求计划、逐字证据、逐问题覆盖和两个已读来源交叉核验", async () => {
    ...
    expect(
      await service.finalize(
        ...
        { ...toolContext, allowPartial: false },
      ),
  ...
  it("记录不可用来源，并只在预算许可且写明限制时接受部分完成", async () => {
    ...
    ).toMatchObject({ status: "unusable", error: "页面需要登录。" })
    const finalization = {
      outcome: "partial" as const,
      questions: [{ id: "q1", status: "uncovered" as const, note: "页面不可访问" }],
      limitations: ["目标页面需要登录，未能取得可核查正文"],
    }
    expect(await service.finalize(finalization, { ...toolContext, allowPartial: false })).toMatchObject({
      status: "blocked",
      issues: expect.arrayContaining([expect.stringContaining("预算")]),
    })
    expect(await service.finalize(finalization, { ...toolContext, allowPartial: true })).toMatchObject({
      status: "partial",
