...
export function AppShell({ appInfo }: AppShellProps) {
  ...
  const selectOutline = (index: number, line: number) => {
    if (editorMode === "rich") {
  ...
  return (
    <div className="relative flex h-screen min-h-0 bg-sidebar text-foreground">
      {sidebarOpen && compact ? (
        <button
          type="button"
      ...
      >
        ...
        <div className="relative flex min-h-0 flex-1">
          ...
          />
          {agentOpen ? <AgentSidebar document={activeDocument} onClose={() => setAgentOpen(false)} /> : null}
        </div>
      </main>
    </div>
  )
