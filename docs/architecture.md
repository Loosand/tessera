...
- **部分实现**：主导航「新任务」与文档 AI 侧栏复用同一 TaskPage、流式协议和消息历史；界面只提供自动/研究/写作/问答创作方式，并支持显式当前文档草稿/图片附件、来源、工具状态、消息内持久化失败与重试、内容 Operation 聚合、文件引用跳转、任务/运行事件恢复、统一 Streamdown Markdown 和渲染 Diff 审批。所有任务已使用共用动态配置的 AI SDK Agent，旧 Chat/Agent 字段只暂存内部资源作用域；自动意图识别和动态资源关联仍待实现。
...
只有独立的安全边界、运行时、依赖或测试生命周期出现后，才新增包。当前领域服务由 Electron 主进程持有；出现第二个真实宿主或独立生命周期前，不建立占位核心包。应用依赖包，包不反向依赖 `apps/`。
