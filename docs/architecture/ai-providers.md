...
自动、研究与写作方式会在请求期模型路由确认原生搜索可用时启用 `web_search`，问答预设始终关闭；renderer 只用同一纯策略解析器做发送前预检，IPC 不再接受 `reasoning` / `webSearch` 能力命令，主进程会根据已保存模型事实重新生成实际 RunPolicy。同一搜索工具既可进入无工作区任务，也可与工作区工具共同进入 Agent 的工具循环。主进程保留工具输出与 URL 来源 Part；renderer 优先按 AI SDK provider-executed 工具的标准 `output.action` / `output.sources` 读取查询、打开页面和来源，同时兼容旧供应商的 `input.query` 与结果数组，按同一助手消息聚合真实搜索次数、去重来源和执行状态，过滤非 http(s) URL 与供应商内部调用标识，并以可展开过程轨迹呈现。每个来源先通过 HTTPS 请求站点 `/favicon.ico`，失败后请求 Favicon.im，二者均失败时回退通用图标；图片懒加载且不发送 Referer。自动与写作每轮最多搜索 5 次，研究 Skill 每轮最多 15 次，运行时再以 20 次作为防御性硬上限；额度耗尽由工具结果表达，不能因供应商兼容差异升级为整轮 Schema 崩溃。供应商用于续轮引用的加密内容保留在工具历史中，不进入可见界面；没有真实工具 Part 时不根据 reasoning 文本伪造搜索活动。供应商原始校验错误在公开前映射为可操作文案，不向 renderer 暴露 Schema、堆栈或凭据。
