...
- `publish-research-plan` 只在研究 Skill 下注册。显式研究由运行时在计划前只开放核心消歧与计划工具，并把目标、范围、交付物和最多八个稳定问题持久化；计划存在后才开放搜索与阅读。自动方式中的简单事实问答无需制造计划，详见[研究工作流与证据链](research-workflow.md)。
...
| 研究 | `$research` | 必要时结构化澄清，要求多步研究计划，核验来源并区分事实/推断/不确定性 | 工作区读取、网络搜索 |
| 写作 | `$writing` | 区分起草/续写/改写/审稿，整理事实与判断，在保持作者声音和修改边界的前提下交付并质检 Markdown | 工作区读取、工作区写入 |
| 问答 | 无 | 不联网，使用对话历史与显式附件直接回答 | 无 |

研究正文已接通澄清、计划、网页深读、证据登记和完成检查工具；Skill 只描述方法，计划顺序、搜索/阅读分层、证据合法性和完整/部分结果由研究领域状态与完成门槛保证。写作正文已实现写作契约、材料分类、文体选择、受控修订和四层质检。当前写作质检仍是 Skill 内的模型工作流，尚未建立独立行为评测集；后续增加固定样本、人工评分和更多专用工具时，不改变注册、持久化、权限和注入协议。

## 管理界面

一级导航中的“技能”页面组合内置注册表和用户安装目录，不维护另一份用户展示清单。当前既可添加单个本地 Skill，也可扫描一个上级目录，在对话框中检查相对位置、描述、有效性、重复/已安装状态，勾选后批量安装；页面继续支持搜索并选择内置/用户 Skill，查看来源、权限边界、安装统计和渐进式加载流程。用户 Skill 可启停、删除，也可以直接带着已选 Skill 创建任务。任务输入浮层同时列出当前已启用且可用的用户 Skill；选择只影响下一轮，界面始终不要求选择 Chat/Agent、联网或思考强度。
...
- 按[研究工作流与证据链](research-workflow.md)继续建设 P1 来源推荐/保存和研究工作文档、P2 浏览器后备；研究 Skill 继续只维护方法论和来源判断规则。
