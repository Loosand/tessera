...
>
> 状态：统一交互、消息内失败、内容 Operation 聚合与混合方案首轮 dev Electron 闭环已实现，最终内容存储仍处于探索阶段。所有任务共用 AI SDK `ToolLoopAgent`、
...
当前研究策略已经能注册原生搜索和计划展示工具，但还没有把计划、页面阅读、证据与完成条件固化为运行时阶段。
这部分不应继续堆进统一 Agent Prompt，而按[研究工作流与证据链](research-workflow.md)作为领域工作流补齐；底层仍复用
同一个 `ToolLoopAgent`、消息 Part、审批和恢复协议。
...
- 生成失败作为类型化 `data-task-error` Part 留在助手消息中，与已经收到的部分正文一起持久化；最后一次失败可从
  消息内重试，输入框不重复显示同一错误。
- 工具错误应说明哪个动作没有完成以及文件当前真实位置，不能只显示模型或工具内部异常。
...
当前自动化已覆盖“普通对话 → 写作 Skill → 创建文档 → 创建项目 → 移动文档 → 结构检查 → 关闭并重开 SQLite”的
混合方案闭环，验证同一 Task 的消息、Artifact、目标 Workspace、Markdown 和 Operation 审计能够恢复；显式研究的
计划、页面阅读与证据链仍由独立研究工作流阶段补齐，不能把这条较窄验收等同于完整研究闭环。
