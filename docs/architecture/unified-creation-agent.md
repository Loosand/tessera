...
固定累计 Token 或 Tessera 自造的单步输出额度作为收尾条件；已知模型显式采用档案声明的原生最大输出，避免兼容
SDK 回落到 4096，上下文窗口只用于放宽异常循环保险丝。跨进程守卫与 Agent 执行 Schema 对 `maxSteps`、`timeoutMs`
和非空 `maxOutputTokens` 共用正安全整数边界，损坏或非有限的持久化策略不得进入运行时。
...
| 研究 | 深度推理、原生联网和来源核验；目标状态执行显式计划、搜索、阅读、证据与完成检查 | 缺少可靠联网能力时明确阻止或请求改用自动，不伪造研究 |
...
负责；底层仍复用同一个 `ToolLoopAgent`、消息 Part、审批和恢复协议。
...
- 生成失败作为带稳定 code、phase 与 retryable 的版本化 `data-task-error` Part 留在助手消息中，与已经收到的部分
  正文一起持久化；AI SDK 字符串化前先从嵌套供应商异常提取稳定类别和可安全公开的 HTTP 状态，原始响应体与凭据不
  进入 renderer。最后一次失败可从消息内重试：若已有成功 Tool Part，新 request 复用这些结果并从其后继续，
  `continuedFromMessageId` 进入运行资源摘要；研究重试还创建带 provenance 的新 request 并继承计划、来源、证据与
  推荐，不从空白开始。输入框不重复显示同一错误。
...
8. **产物协作界面（已实现基础闭环）**：Artifact 卡片可打开文档 + 原 Task 侧栏，移动后追加新的 scope 关系；
   同一回复中的创建文档、创建项目、移动文档和结构检查已经聚合为一条 Operation 活动，并保留 AI SDK 原生审批面板。
9. **收敛旧模式（部分实现）**：用户路径和模型运行已无 Chat/Agent 选择；历史 `mode/workspace_id` 仍用于兼容归属，
   当前轮工作区工具必须同时满足眼下工作区授权与请求相关性；纯公开问题不会因处于工作区而获得本地工具。
10. **运行时加固与验收（已实现）**：统一版本化运行/工具失败、恢复/取消/乱序事件和 AI SDK 消息类型边界；提供
    脱敏单次运行解释、流增量合并、研究状态续跑与已完成工具结果续跑，并以磁盘 SQLite 验收普通问答、研究工具链、
    写作、创建/移动内容及重启中断收口；真实 FKJ 供应商运行另通过领域黄金审计。
...
2. 同一会话下一轮要求深入研究，系统发布研究计划，区分搜索与页面阅读，显示证据覆盖和来源，并保留上一轮历史。
...
当前自动化已覆盖“普通问答 → 研究 Skill 的计划/搜索/网页读取事件 → 写作 Skill → 创建文档 → 创建项目 → 移动文档
→ 关闭并重开 SQLite → 收口遗留运行”的统一运行时闭环，验证同一 Task 的消息、每轮运行解释、Artifact、目标
Workspace、Markdown 和 Operation 审计能够恢复，已执行的写操作不会在恢复时重放。独立研究工作流的确定性测试
覆盖证据逐字校验、跨来源完成门槛、推荐/保存材料与写作交接；真实 FKJ 续跑进一步核验计划 provenance、系统代理
Reader、不可用页面、15 条证据、来源推荐和最终报告的一致性。
