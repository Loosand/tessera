...
- 侧栏顶部 Space 菜单固定把默认空间放在第一项，随后列出最近文件工作区和系统目录选择入口；侧栏主体连续显示当前 Space 的最近任务并提供每页 6 条的紧凑分页，文件 Space 再显示真实目录/Markdown 文件树。“全部任务”一级页面通过同一个当前 Space IPC 契约按每页 12 条查询总数和内容，必须越过旧 100 条列表上限，不得在已截断数组上假分页。打开任务或文档只切换主画布，不再进入工作区二级侧栏；空任务页在同一作用域展示最多五个最近任务。
