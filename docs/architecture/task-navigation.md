...
任务是 Chat 与 Agent 共用的产品会话。Chat/Agent 是内部运行与授权事实，不再是创作用户必须选择的产品模式：独立任务使用 Chat，工作区任务默认使用 Agent。任务导航只决定会话归属和恢复位置，不向模型隐式提供工作区正文；当前文档必须作为显式附件加入，普通 Chat 和 Agent 才会收到对应草稿内容。
...
- 创作模式与内部 mode 正交：`null` 表示自动，`research` / `writing` 对应两份内置 Skill，`question-answering` 是不加载 Skill 的问答行为标记。四种选择集中在创作模式浮层，任务首次保存后不可切换。
