...
>
> 状态：部分实现。无工作区与工作区任务均使用同一 AI SDK `ToolLoopAgent` 和类型化 call options / `prepareCall`；逐轮 Skill、保守自动意图、受信任 RunPolicy、动态资源、Artifact、内容项目工具、问题暂停/续跑、研究计划、研究状态续跑、回答后引申问题、Markdown Diff、MCP 审批、运行恢复和消息历史已实现。Shell、通用步骤级 durable replay 与多窗口接管尚未实现。
...
- 当前文档继续作为可见、可移除附件加入；进入文档侧栏可以默认附加编辑器最新草稿，但不能静默读取未附加正文。侧栏顶部显示当前任务并可切换同工作区历史或新建任务；通过 Artifact 切换到产物项目时保留原任务，工作区资源刷新只更新可选任务列表，不重置当前会话。
