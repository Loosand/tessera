...
- [x] 正文、reasoning 与 Agent 结果预览统一使用 Streamdown，支持 GFM、CJK、Shiki、KaTeX、Mermaid、流式未闭合语法修复与逐字符过渡；代码、表格和图表提供复制、下载、全屏等适用控件，工作区 Markdown 引用继续可跳转，原始 HTML 与危险协议保持禁用。
- [x] 将 reasoning、正文、来源和工具调用拆为独立 Part 呈现；reasoning 默认展开、可折叠并以紧凑密度复用正文 Streamdown 渲染器，正文使用 SDK Part 状态启用增量光标和动画，工具状态不混入 Markdown。
