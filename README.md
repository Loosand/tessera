...
工作区和正文默认保存在本地，用户自选模型与供应商；Agent 的文件操作受工作区、版本与执行环境约束，运行结果可以审查。

## 为什么是 Tessera

- **本地数据属于你**：Markdown 是正文事实源，文件保存在用户选择的目录中，不锁进 Tessera 的专有格式。
- **Agent 不能越过工作区边界**：`read/edit/write` 受路径、版本与原子提交保护；受控 Bash 只在平台隔离探针通过时启用，拒绝网络、宿主 Secret 和工作区外访问。
...
材料从哪里来、Agent 做了什么、哪些内容发给了模型，以及文件和运行事实能否被检查与恢复。
...
| Agent 写入 | 用户打开工作区后，`edit/write` 在路径和版本边界内直接提交；越界、外部冲突或取消不会覆盖磁盘新版本。 |
| Agent Bash | 仅在 macOS Seatbelt 探针通过时注册；前台命令按本轮策略只读或读写工作区，不继承宿主 Secret，网络默认拒绝。 |
...
- **部分实现**：普通问答、研究、写作和内容整理已接入统一 Agent 运行时；本地核心已收敛为 `read/edit/write/bash`，
  其中 Bash 当前只在 macOS 真实隔离探针通过时启用。精确编辑、冲突复核、原子写入、运行事件可靠性、事实
  Progress、脱敏 Execution Context、Artifact 关系/预览和旧审批只读兼容已落地；MCP 工具仍逐次批准。
- **下一阶段**：[Agent 减法路线图](docs/architecture/agent-simplification-roadmap.md)一期 P0–P5 已收口；
  [`tessera-core` Agent Eval](docs/quality/agent-eval-method.md) 已把首批六类黄金任务、人工质量和客观效率固化为代码资产，
  后续先接真实模型 runner 建立基线，再评估材料采集、本地版本历史、Artifact Diff 和 Git 工作区支持。
...
- [Agent 减法实施路线图](docs/architecture/agent-simplification-roadmap.md)
