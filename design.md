...
| 区块化即时预览编辑 | TipTap | 以语义块组织交互；Markdown 仍是内容事实源，编辑器状态不能替代文件 |
...
- 即时预览编辑以区块为主要交互单位：用户可以插入、选择、转换、拖动、复制和删除语义块，同时继续在块内
  直接编辑文本；普通状态显示排版结果，需要消除语法歧义时才就地显示必要的 Markdown 标记。
- 区块是编辑器运行时的交互投影，不额外引入块 JSON 作为内容副本。跨会话身份、协作引用或 Agent 定位如需
  持久块 ID，必须先定义可由 Markdown 表达和审查的协议。
- 工作区与设置视图预先挂载并通过显隐切换，已激活的编辑表面也在模式切换时保持挂载；达到 100,000 字符的大文档默认使用源码模式，并提供明确的即时预览覆盖入口。
